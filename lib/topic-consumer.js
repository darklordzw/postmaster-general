...
class TopicConsumer extends EventEmitter {
	...
	constructor(connection, topic, callback, options) {
		...
		this.channel = this.connection.createChannel({
			json: true,
			setup: async (channel) => {
				if (this.queue.deadLetterExchange) {
					await channel.assertExchange(this.queue.deadLetterExchange, 'topic');
				}
	...
	get outstandingMessageCount() {
		return this.outstandingMessages.size;
	}
	...
	async startConsuming() {
		...
		await this.channel.addSetup(this.consumeSetup);
	}
	...
	async stopConsuming() {
		if (this.consumeSetup) {
			...
			this.consumeSetup = null;
			await this.channel.removeSetup(consumeSetup, async (channel) => {
